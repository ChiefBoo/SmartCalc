...
int check_validation(char *expression) {
  ...
  for (int i = 0; i < len_expression && brace_count >= 0 && len_expression > min_len && status == OK; i++) {
    ...
    if (strchr("%.*/^+-(", expression[len_expression-1])) {
      ...
      status = ERROR;
    } else if (i > 0 && strchr("(%.*/^+-", expression[i-1]) && strchr(")%.*/^", expression[i])) {
      status = ERROR;      
    } else if (strchr("sctlam", expression[i])) {
      status = check_funcs(expression, &i, &len_expression);
    } else if (strchr("+-", expression[i]) && i > 0 && strchr("%.*/^+-", expression[i-1])) {
      status = ERROR;
    } else if ((strchr("+-", expression[i]) && i > 0 && expression[i-1] == '(')) {
      memmove( expression + i + 1, expression + i, strlen( expression + i) + 1);
      memset( expression + i, '0', 1);
      len_expression++;
      i++;
    } else if (expression[i] == 'x') {
      if (i == 0 && (strchr("().sctal", expression[i+1]) || is_digit(expression, i+1))) {
        status = ERROR;
      } else if (i > 0 && (i != (len_expression - 1)) && (strchr("(.sctal", expression[i+1]) || strchr(").", expression[i-1]) || is_digit(expression, i+1) || is_digit(expression, i-1))) {
        status = ERROR;
      } else if (i == (len_expression - 1) && (strchr("().", expression[i-1]) || is_digit(expression, i-1))) {
        status = ERROR;
      }
    } 
  }
  if (len_expression < min_len || brace_count > 0 || status == ERROR)
    status = ERROR;
  return status;
}
...
int check_funcs(char* expression, int* idx, int* len) {
  ...
  if (dest[0] == 'a') {
    ...
    if (!strncmp(new_dest, "asin(", 5) || !strncmp(new_dest, "acos(", 5) || !strncmp(new_dest, "atan(", 5)) {
      *idx += 3;
    } else {
      status = ERROR;
    }
    ...
    if (strlen(dest) > 2 && dest[0] == 's' && dest[1] == 'q') {
      ...
      if (!strncmp(new_dest, "sin(", 4) || !strncmp(new_dest, "cos(", 4) || !strncmp(new_dest, "tan(", 4)) {
        *idx += 2;
      } else {
        status = ERROR;
      }
    ...
    if (!strncmp(new_dest, "mod", 3)) {
      expression[*idx] = '%';
      memmove(expression+(*idx)+1, expression+(*idx)+3, strlen(expression+(*idx) + 3) + 1);
      *len = *len-2;
    } else {
      status = ERROR;
    }
...
int is_digit(char *expression, int idx) { return (expression[idx] >= '0' && expression[idx] <= '9'); }

//parser
void reverse_stack(Node** old_stack, Node** new_stack) {
    while (*old_stack) {
      push(new_stack, (*old_stack)->lex.type, (*old_stack)->lex.priority, (*old_stack)->lex.value);
      pop(old_stack);
    }
}

void parser(char* expression, Node** stack) {
  int len_expression = strlen(expression);
  ...
  for (int i = 0; i < len_expression; i++) {
    if (strchr(number_type, expression[i])) {
      ...
      number[j] = '\0';
      push(stack, NUM, get_priority(NUM), atof(number));
      number[0] = '\0';
      i--;
    } else if (strchr(arithmetic_type, expression[i])) {
        if (expression[i] == '+') push(stack, ADD, get_priority(ADD), expression[i]);
        else if (expression[i] == '-') push(stack, DIF, get_priority(DIF), expression[i]);
        else if (expression[i] == '*') push(stack, MUL, get_priority(MUL), expression[i]);
        else if (expression[i] == '/') push(stack, DIV, get_priority(DIV), expression[i]);
        else if (expression[i] == '%') push(stack, MOD, get_priority(MOD), expression[i]);
        else if (expression[i] == '^') push(stack, POW, get_priority(POW), expression[i]);
    } else if (strchr(func_type, expression[i])) {
      int func = get_func(expression, &i);
      if (func == 's') push(stack, SIN, get_priority(SIN), 's');
      else if (func == 'c') push(stack, COS, get_priority(COS), 'c');
      else if (func == 't') push(stack, TAN, get_priority(TAN), 't');
      else if (func == 'S') push(stack, ASIN, get_priority(ASIN), 'S');
      else if (func == 'C') push(stack, ACOS, get_priority(ACOS), 'C');
      else if (func == 'T') push(stack, ATAN, get_priority(ATAN), 'T');
      else if (func == 'L') push(stack, LOG, get_priority(LOG), 'L');
      else if (func == 'n') push(stack, LN, get_priority(LN), 'l');
      else if (func == 'q') push(stack, SQRT, get_priority(SQRT), 'q');
    } else if (expression[i] == '(') push(stack, BRACK_O, get_priority(BRACK_O), '(');
    else if (expression[i] == ')') push(stack, BRACK_C, get_priority(BRACK_C), ')');
    else if (expression[i] == 'x') push(stack, X, get_priority(X), 'x');
  }
}
...
int get_priority(type_t type) {
  ...
  if (type == COS || type == SIN || type == TAN || type == ACOS
    || type == ASIN || type == ATAN || type == SQRT || type == LN || type == LOG) priority = 0;
  return priority;  
}
...
int is_func_binary(type_t type) {
  int is_binary = 0;
  if (type == ADD || type == DIF || type == MUL || type == DIV ||
      type == MOD || type == POW)
    is_binary = 1;
  return is_binary;
}
...
void reverse_polish_notation(Node** stack, Node** output_stack) {
  ...
  while ((*stack) != NULL) {
    if ((*stack)->lex.type == NUM || (*stack)->lex.type == X) {
      push(output_stack, (*stack)->lex.type, (*stack)->lex.priority, (*stack)->lex.value);
      pop(stack);
    } else if (is_func_unary((*stack)->lex.type) || (*stack)->lex.type == BRACK_O) {
      push(&help_stack, (*stack)->lex.type, (*stack)->lex.priority, (*stack)->lex.value);
      pop(stack);
    } else if ((is_func_binary((*stack)->lex.type))) {
      while(help_stack != NULL && (is_func_binary((help_stack)->lex.type)) && help_stack->lex.priority >= (*stack)->lex.priority) {
        push(output_stack, help_stack->lex.type, help_stack->lex.priority, help_stack->lex.value);
        pop(&help_stack);
      }
      push(&help_stack, (*stack)->lex.type, (*stack)->lex.priority, (*stack)->lex.value);
      pop(stack);
...
int calculate(Node** notation, double x_value, double* result) {
  ...
  while ((*notation) != NULL && status == OK) {
    if ((*notation)->lex.type == NUM || (*notation)->lex.type == X ) {
      ...
      push(&number_stack,(*notation)->lex.type, (*notation)->lex.priority, (*notation)->lex.value);
      pop(notation);
    } else {
      if (is_func_binary((*notation)->lex.type)) {
        ...
        pop(&number_stack);
        if (((*notation)->lex.type == MOD || (*notation)->lex.type == DIV) && fabs(num2) < eps) {
          status = ERROR;
        } else {
        ...
        double num1 = number_stack->lex.value;
        pop(&number_stack);
        if (((*notation)->lex.type == SQRT || (*notation)->lex.type == LN || (*notation)->lex.type == LOG)
          && (fabs(num1) < eps || num1 < 0)) {
          status = ERROR;
        } else if (((*notation)->lex.type == ACOS || (*notation)->lex.type == ASIN)
          && (num1 > 1 || num1 <-1)) {
          status = ERROR;
        } else if (((*notation)->lex.type == TAN) && (fabs(cos(num1)) < eps)) {
          status = ERROR;
        } else {
